...
#         import pdb; pdb.set_trace()
        ...
        if len(da_int.dims) == 1 and da_int.cf['Z'].name in da_int.dims:
            da_int = da_int.swap_dims({da_int.cf['Z'].name: da_int.cf['vertical'].name})
            ...
            for i in range(len(da_int[dim_var_name])):
                new_da.append(da_int.isel({dim_var_name: i}).swap_dims({da_int.cf['Z'].name: da_int.cf['vertical'].name}).cf.interp(vertical=Z))
            da_int = xr.concat(new_da, dim=dim_var_name)
        elif len(da_int.dims) > 2:
            # need to implement (x)isoslice here
            raise NotImplementedError("Currently it is not possible to interpolate in depth with more than 1 other (time) dimension.")
...
def sel2d(var, lons, lats, lon0, lat0, inds=None, T=None, iT=None, Z=None, iZ=None):
    ...
    if len(inds) == 2:  # structured models
        isel['X'] = X=inds[1]
        isel['Y'] = X=inds[0]
    elif len(inds) == 1:  # unstructured models
        isel['X'] = inds[0]
    else:
    ...
    if Z is not None:
        if len(result.dims) == 1 and result.cf['Z'].name in result.dims:
            ...
            for i in range(len(result[dim_var_name])):
                new_results.append(result.isel({dim_var_name: i}).swap_dims({result.cf['Z'].name: result.cf['vertical'].name}).cf.sel(vertical=Z, method='nearest'))
