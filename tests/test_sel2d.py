...
def test_lon_lat_types():
    ...
    # Floats
    da_test = em.sel2d(da, lon_rho=lon, lat_rho=lat, use_xoak=False).squeeze()
    ...
    # Array
    da_test = em.sel2d(
        da, lon_rho=np.array([lon]), lat_rho=np.array([lat]), use_xoak=False
    ).squeeze()
    assert np.allclose(da_check, da_test)
...
def test_index_reuse():
    ...
    t1temp = time()
    em.sel2d(da, lon_rho=lon, lat_rho=lat, use_xoak=False).squeeze()
    t1 = time() - t1temp
...
def test_ll_name_reversal():
    ...
    da1 = em.sel2d(da, lon_rho=lon, lat_rho=lat, use_xoak=False).squeeze()
    da2 = em.sel2d(da, lat_rho=lat, lon_rho=lon, use_xoak=False).squeeze()
    assert np.allclose(da1, da2)


# sel2d only selects in lonlat, so no need to test sel2d with time
# def test_sel_time():
...
#     da_test = em.sel2d(da, lon_rho=lon, lat_rho=lat, ocean_time=0, use_xoak=False)
#     import pdb; pdb.set_trace()
#     assert np.allclose(da_check, da_test)

#     # Won't run in different input order
#     with pytest.raises(ValueError):
#         em.sel2d(da, ocean_time=0, lon_rho=lon, lat_rho=lat, use_xoak=False)


def test_cf_versions():
    """Test cf name versions of everything"""

    da_check = em.sel2d(da, lon_rho=lon, lat_rho=lat, use_xoak=False)
    da_test = em.sel2dcf(da, longitude=lon, latitude=lat, use_xoak=False)
    assert np.allclose(da_check, da_test)  # )

    da_test = em.sel2dcf(da, latitude=lat, longitude=lon, use_xoak=False)
    assert np.allclose(da_check, da_test)  # )

    da_check = em.sel2d(da, lon_rho=lon, lat_rho=lat, ocean_time=0, use_xoak=False)
    da_test = em.sel2dcf(da, latitude=lat, longitude=lon, T=0, use_xoak=False)
    assert np.allclose(da_check, da_test)  # )

    da_test = em.sel2dcf(da, T=0, longitude=lon, latitude=lat, use_xoak=False)
    assert np.allclose(da_check, da_test)  # )
